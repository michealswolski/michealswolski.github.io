...
export default function App(){
  ...
  const cls=[{label:"GitHub",href:"#"},{label:"LinkedIn",href:"https://linkedin.com/in/michealswolski"},{label:"Email",href:"mailto:"+P.email},{label:"Download Resume",href:"#",isResume:true}];
    ...
    {booted&&<>
      ...
      <section id="about" style={{position:"relative",zIndex:2,minHeight:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",padding:"130px clamp(20px,5.5vw,72px) 90px",transform:"translateY("+px+"px)"}}>
        ...
        <div style={{display:"flex",gap:"clamp(20px,3.5vw,44px)",marginTop:56,flexWrap:"wrap",opacity:loaded?1:0,transform:loaded?"none":"translateY(18px)",transition:"all 1s cubic-bezier(.22,1,.36,1) 1.1s"}}>
          {[{v:"23+",l:"Projects Built"},{v:"9",l:"Security Labs"},{v:"5",l:"OEM Divisions"},{v:"4",l:"Personal Apps"}].map(s=><div key={s.l}><div style={{fontFamily:"var(--h)",fontSize:26,fontWeight:800,color:th.text,lineHeight:1}}>{s.v}</div><div style={{fontFamily:"var(--m)",fontSize:9,color:th.textFaint,letterSpacing:1.5,marginTop:3,textTransform:"uppercase"}}>{s.l}</div></div>)}
        </div>
      </section>
      ...
      <section id="projects" style={{position:"relative",zIndex:2,padding:"72px clamp(20px,5.5vw,72px) 90px",transform:"translateY("+(-scrollY*0.02)+"px)"}}>
        <SL th={th}>Projects</SL><ST th={th} sub="Enterprise deployments, personal GitHub repos, and hands-on cybersecurity labs. Click any card for details.">Engineering Portfolio</ST>
      ...
      {/* EXPERIENCE */}
      <section id="experience" style={{position:"relative",zIndex:2,padding:"72px clamp(20px,5.5vw,72px) 90px"}}><SL th={th}>Experience</SL><ST th={th} sub="Internship at a Tier 1 automotive supplier supporting major OEM cybersecurity programs.">Work & Internships</ST>
